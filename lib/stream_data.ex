defmodule StreamData do
  ...
  basis of property-based testing. When a generator is called internally, it will
  generate a value that is not a directly usable value. Instead, it's a
  "wrapper" around a normal value that is optimized for property-based testing and,
  ...
  Shrinking is used for property-based testing, where it's important to find the
  minimal value for which a property fails (since generated values are often
  bigger and full of garbage). See `ExUnitProperties` for more information.
  ...
  shrinked list will have more than `:min_length` elements.
  """
  @spec unfold(acc_t, (acc_t -> {t(data_t), acc_t}), Keyword.t) :: t([data_t]) when acc_t: var, data_t: var
  def unfold(next_acc, next_fun, options) do
    ...
    new(fn seed, size ->
      ...
      next_acc
      |> do_unfold(next_fun, next_seed, size, length, [])
      |> LazyTree.zip()
      |> LazyTree.map(&list_lazy_tree(&1, min_length))
      |> LazyTree.flatten()
    end)
  ...
  The same as calling `unfold/3` with `[]` as options.
  """
  @spec unfold(acc_t, (acc_t -> {t(data_t), acc_t})) :: t([data_t]) when acc_t: var, data_t: var
  ...
  @compile{:inline, do_unfold: 6}
  defp do_unfold(_next_acc, _next_fun, _seed, _size, 0, result), do: result
  defp do_unfold(next_acc, next_fun, seed, size, length, result) do
    {seed1, seed2} = split_seed(seed)
