defmodule ExUnitProperties do
  ...
  By default, the generation size (see the "Generation size" section in the documentation for
  `StreamData`) is chosen at random between in `1..100`. If you want finer control over the
  generation size:

  * Use `StreamData.resize/2` to resize `data`;

  * Use `StreamData.scale/2` to scale the generation size; or

  * `config :stream_data, pick_generation_size: 1..5` to set a different range for `pick/1`.
  ...
  def pick(data) do
    ...
    size = Application.fetch_env!(:stream_data, :pick_generation_size)

    pick(data, seed, size)
  end

  defp pick(data, seed, 1..n) do
    {size, seed} = :rand.uniform_s(n, seed)
    pick(data, seed, IO.inspect(size, label: "size"))
  end

  defp pick(data, seed, size) when is_integer(size) do
